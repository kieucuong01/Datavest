...
scp @scpArgs $archive $checksum "${target}:/opt/datavest/incoming/"
...
ssh @sshArgs $target "/usr/local/bin/deploy-datavest /opt/datavest/incoming/$(Split-Path -Leaf $archive) $sha"
