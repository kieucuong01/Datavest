...
CREATE TABLE IF NOT EXISTS qd_worker_heartbeats (
    ...
    updated_at TIMESTAMP NOT NULL DEFAULT NOW(),
    CHECK (role IN ('api', 'scheduler', 'celery', 'celery-beat')),
...
CREATE TABLE IF NOT EXISTS qd_agent_tokens (
    ...
    scopes TEXT NOT NULL DEFAULT 'R',            -- comma-separated subset of R,W,B,N
    markets TEXT NOT NULL DEFAULT '*',           -- comma-separated allowlist or '*'
    instruments TEXT NOT NULL DEFAULT '*',       -- comma-separated allowlist or '*'
    paper_only BOOLEAN NOT NULL DEFAULT TRUE,    -- paper portfolio boundary
...
-- Paper-only ledger for Agent portfolio simulations.
