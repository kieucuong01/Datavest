...
from app.runtime import worker_heartbeat
...
def test_mfa_start_setup_returns_user_id_not_missing_id(monkeypatch):
    ...
    monkeypatch.setattr(mfa_service, "encrypt_secret_blob", lambda secret: f"encrypted:{secret}")
...
def test_worker_heartbeat_returns_its_real_primary_key(monkeypatch):
    conn = _CaptureConn()
    monkeypatch.setattr(
        worker_heartbeat,
        "get_db_connection",
    ...
    worker_heartbeat.WorkerHeartbeat(
        "celery",
        worker_id="celery:test",
    ).record_running()

    sql, params = conn.cursor_obj.calls[0]
    assert "RETURNING worker_id" in sql
    assert "RETURNING id" not in sql
    assert params == ("celery:test", "celery", "{}")
    assert conn.committed
