...
def _agent_route_operations(app) -> set[tuple[str, str]]:
    operations: set[tuple[str, str]] = set()
    for rule in app.url_map.iter_rules():
        if not rule.rule.startswith("/api/agent/v1") or rule.rule.startswith("/api/agent/v1/me/"):
            continue
...
def test_agent_openapi_matches_registered_route_source(app):
    ...
    assert _agent_route_operations(app) == documented
