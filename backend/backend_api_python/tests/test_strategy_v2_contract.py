...
def test_instrument_parser_normalizes_ptrade_and_crypto_symbols():
    assert parse_instrument("VNStock:FPT").key == "VNStock:FPT"
...
"""
    ...
    assert [item.symbol for item in manifest.universe.instruments] == ["FPT", "VCB"]
...
def test_manifest_rejects_removed_index_universe():
    code = """
def initialize(context):
...
"""
    with pytest.raises(StrategyV2ContractError, match="indexUniverseUnsupported"):
        compile_strategy_v2(code)
...
def test_manifest_rejects_removed_index_fundamental_universe():
    code = """
def initialize(context):
