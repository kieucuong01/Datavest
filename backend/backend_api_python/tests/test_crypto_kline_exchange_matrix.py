...
def test_unscoped_crypto_kline_falls_back_to_another_public_exchange(monkeypatch):
    ...
    source._symbol_for_public_market = lambda _symbol: "BTC/USDT"
    ...
    monkeypatch.setattr(
        CryptoDataSource,
        "_for_public_exchange",
        classmethod(
            lambda _cls, market_type, exchange_id, *, allow_fallback: (
...
def test_unscoped_swap_kline_fallback_preserves_market_type(monkeypatch):
    ...
    source._symbol_for_public_market = lambda _symbol: "BTC/USDT:USDT"
    source._is_invalid_symbol_cached = lambda _symbol: False
    source._fetch_ohlcv = lambda *_args, **_kwargs: []
...
def test_public_ticker_reuses_kline_promoted_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        CryptoDataSource,
        "_for_public_exchange",
        classmethod(lambda _cls, market_type, exchange_id, *, allow_fallback: FallbackSource()),
    )
...
def test_factory_uses_public_source_for_default_and_preferred_exchange(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "for_public_market",
        classmethod(
            lambda _cls, market_type, preferred_exchange_id="": (
                calls.append((market_type, preferred_exchange_id))
                or (preferred_source if preferred_exchange_id else default_source)
            )
        ),
    )

    assert DataSourceFactory._resolve_source("Crypto", market_type="swap") is default_source
    assert DataSourceFactory._resolve_source(
        "Crypto",
        exchange_id="gate",
        market_type="swap",
    ) is preferred_source
    assert calls == [("swap", ""), ("swap", "gate")]


def test_public_crypto_source_with_fallback_disabled_does_not_cross_exchange(monkeypatch):
    source = object.__new__(CryptoDataSource)
    ...
    monkeypatch.setattr(
        ...
        "_for_public_exchange",
        classmethod(
            lambda _cls, market_type, exchange_id, *, allow_fallback: attempts.append((exchange_id, market_type))
        ),
    )
