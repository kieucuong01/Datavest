...
@pytest.mark.parametrize("stored_scopes", ["R,T", "R,C", "R,B,T,C"])
def test_legacy_token_with_removed_scope_is_rejected_before_whoami(
    client, monkeypatch, stored_scopes
):
    """A legacy token row must fail closed instead of silently losing T/C."""
    agent_auth._schema_ready = True
    monkeypatch.setattr(
        agent_auth,
        "_lookup_token",
        lambda _raw: _fake_token_row(scopes=stored_scopes),
    )
    monkeypatch.setattr(agent_auth, "_touch_token_last_used", lambda *_: None)
    monkeypatch.setattr(agent_auth, "_audit", lambda *args, **kwargs: None)

    response = client.get("/api/agent/v1/whoami", headers=_bearer())

    assert response.status_code == 403
    payload = response.get_json()
    assert "unsupported scope" in payload["message"].lower()
    assert "data" not in payload or payload["data"] is None
