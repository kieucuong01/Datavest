...
def test_strategy_source_list_omits_code_and_detail_always_returns_visible_source(client, monkeypatch, source_service):
    _authorize(monkeypatch, "R")
    ...
    assert detail.get_json()["data"]["code"] == source_service.source["code"]
    assert "code_hidden" not in detail.get_json()["data"]
