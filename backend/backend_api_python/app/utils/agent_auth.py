...
ALL_SCOPES = (SCOPE_R, SCOPE_W, SCOPE_B, SCOPE_N)
...
def _raw_scope_values(raw: str | Iterable[str] | None) -> set[str]:
    if raw is None:
        return {SCOPE_R}
...
def parse_scopes(raw: str | Iterable[str] | None) -> set[str]:
    return _raw_scope_values(raw).intersection(ALL_SCOPES)
...
def agent_required(scope: str = SCOPE_R):
    ...
    def decorator(fn: Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            raw_scopes = _raw_scope_values(row.get("scopes"))
            unsupported_scopes = raw_scopes.difference(ALL_SCOPES)
            scopes = raw_scopes.intersection(ALL_SCOPES)
            sanitized_row = dict(row)
            sanitized_row["scopes"] = ",".join(sorted(scopes))
            if unsupported_scopes:
                g.agent_token = sanitized_row
                resp, code = _err(
                    403,
                    f"Token contains unsupported scope(s): {', '.join(sorted(unsupported_scopes))}",
                    status=403,
                )
                _audit(
                    scope,
                    403,
                    {"reason": "unsupported_scope", "unsupported": sorted(unsupported_scopes)},
...
def market_allowed(market: str) -> bool:
    try:
        ...
        return False
    row = current_token()
    return list_matches(canonical_market, parse_csv_list(row.get("markets"), default="*"))
