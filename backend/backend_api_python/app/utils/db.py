...
def _resolve_smart_insights_sql_path() -> Path:
    return Path(__file__).resolve().parent.parent.parent / 'migrations' / '20260824_smart_insights_foundation.sql'


def _resolve_smart_insights_production_sources_sql_path() -> Path:
    return Path(__file__).resolve().parent.parent.parent / 'migrations' / '20260825_smart_insights_production_sources.sql'


def _resolve_smart_insights_crypto_pulse_runtime_sql_path() -> Path:
    return Path(__file__).resolve().parent.parent.parent / 'migrations' / '20260825_smart_insights_crypto_pulse_runtime.sql'


def _resolve_smart_insights_legacy_browser_sources_sql_path() -> Path:
...
def _resolve_smart_insights_market_evidence_sql_path() -> Path:
...
def _resolve_production_account_import_sql_path() -> Path:
    return Path(__file__).resolve().parent.parent.parent / 'migrations' / '20260826_production_account_import.sql'


def _resolve_supported_markets_scope_sql_path() -> Path:
...
def _resolve_public_research_reports_sql_path() -> Path:
...
def _apply_init_sql(logger, *, strict: bool = False):
    ...
    try:
        ...
        smart_insights_sql = _resolve_smart_insights_sql_path()
        if smart_insights_sql.exists():
            sql_parts.append(smart_insights_sql.read_text(encoding='utf-8'))
        smart_insights_production_sources_sql = _resolve_smart_insights_production_sources_sql_path()
        if smart_insights_production_sources_sql.exists():
            sql_parts.append(smart_insights_production_sources_sql.read_text(encoding='utf-8'))
        smart_insights_crypto_pulse_runtime_sql = _resolve_smart_insights_crypto_pulse_runtime_sql_path()
        if smart_insights_crypto_pulse_runtime_sql.exists():
            sql_parts.append(smart_insights_crypto_pulse_runtime_sql.read_text(encoding='utf-8'))
        ...
        production_account_import_sql = _resolve_production_account_import_sql_path()
        if production_account_import_sql.exists():
            sql_parts.append(production_account_import_sql.read_text(encoding='utf-8'))
        ...
        if smart_insights_sql.exists():
            total_size += smart_insights_sql.stat().st_size
        if smart_insights_production_sources_sql.exists():
            total_size += smart_insights_production_sources_sql.stat().st_size
        if smart_insights_crypto_pulse_runtime_sql.exists():
