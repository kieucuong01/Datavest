...
def register_human_blueprints(api: Api) -> None:
    ...
    if is_enabled("DATAVEST_COMMUNITY_ENABLED"):
        from app.routes.community import community_blp

        registrations.append((community_blp, "/api/community"))

    if is_enabled("DATAVEST_PORTFOLIO_OPTIMIZER_ENABLED"):
        from app.routes.portfolio_optimizer import portfolio_optimizer_blp

        registrations.append((portfolio_optimizer_blp, "/api/portfolio"))
    ...
    for tag in ALL_TAGS:
        if tag["name"] == "SmartInsights" and not is_enabled("DATAVEST_SMART_INSIGHTS_ENABLED"):
            continue
