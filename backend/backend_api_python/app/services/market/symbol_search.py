...
def dedupe_symbol_results(items: Iterable[dict], limit: int) -> list:
    ...
    for item in items or []:
        raw_market = (item.get("market") or "").strip()
        symbol = (item.get("symbol") or "").strip().upper()
        name = (item.get("name") or "").strip()
        if not raw_market or not symbol:
            continue
        try:
            market = normalize_supported_market(raw_market)
        ...
        asset_class = str(item.get("asset_class") or _default_asset_class(market)).strip().lower()
...
def _search_crypto_exchange(
    ...
    try:
        ...
        from app.data_sources.crypto import apply_public_ccxt_endpoint_config, resolve_public_ccxt_market
