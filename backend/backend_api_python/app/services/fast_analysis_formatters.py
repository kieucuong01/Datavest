...
def build_trend_outlook_summary(trend_outlook: Dict[str, Any], language: str) -> str:
    ...
    def label(trend: str) -> str:
        ...
        if is_vi:
            return {"BUY": "tích cực", "SELL": "tiêu cực", "HOLD": "trung lập/đi ngang"}.get(normalized, "trung lập/đi ngang")
    ...
    if is_vi:
        ...
        def vi_strength(value: Any) -> str:
            return strength_labels.get(str(value or "").lower(), str(value or "trung lập"))

        return " | ".join(
            [
                f"Khoảng 24 giờ: {label(n24.get('trend'))} (cường độ {vi_strength(n24.get('strength'))})",
                f"Khoảng 3 ngày: {label(d3.get('trend'))} (cường độ {vi_strength(d3.get('strength'))})",
                f"Khoảng 1 tuần: {label(w1.get('trend'))} (cường độ {vi_strength(w1.get('strength'))})",
                f"Khoảng 1 tháng: {label(m1.get('trend'))} (cường độ {vi_strength(m1.get('strength'))})",
            ]
        )
    return " | ".join(
        [
