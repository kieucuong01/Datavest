...
class FastAnalysisScoringMixin:
    def _calculate_objective_score(
        ...
        crypto_factor_objective = self._calculate_crypto_factor_score(crypto_factors, price_data, language)
        ...
        fundamental_present = (
            market_type in ("USStock", "VNStock") and _fundamental_meaningful(fundamental)
        )
        ...
        return {
            ...
            "crypto_factor_summary": crypto_factor_objective.get("summary") or "",
        }

    def _localized_crypto_factor_summary(self, crypto_factors: Dict[str, Any], language: str = "en-US") -> str:
        """Build user-facing crypto factor copy without leaking provider-language text."""
        ...
        try:
            volume_change = float(crypto_factors.get("volume_change_24h"))
            if volume_change > 0:
                parts.append("Hoạt động giao dịch tăng" if is_vi else "Trading activity is increasing")
            elif volume_change < 0:
                parts.append("Hoạt động giao dịch giảm" if is_vi else "Trading activity is decreasing")
        except (TypeError, ValueError):
            pass

        try:
            funding_rate = float(crypto_factors.get("funding_rate"))
            if funding_rate > 0:
                parts.append("Phí tài trợ dương" if is_vi else "Funding is positive")
            elif funding_rate < 0:
                parts.append("Phí tài trợ âm" if is_vi else "Funding is negative")
        except (TypeError, ValueError):
            pass

        try:
            oi_change = float(crypto_factors.get("open_interest_change_24h"))
            if oi_change > 0:
                parts.append("hợp đồng mở tăng" if is_vi else "open interest is rising")
            elif oi_change < 0:
                parts.append("hợp đồng mở giảm" if is_vi else "open interest is falling")
        except (TypeError, ValueError):
            pass

        try:
            exchange_flow = float(crypto_factors.get("exchange_netflow"))
            if exchange_flow < 0:
                parts.append("dòng tiền ròng rút khỏi sàn" if is_vi else "net exchange outflow")
            elif exchange_flow > 0:
                parts.append("dòng tiền ròng vào sàn" if is_vi else "net exchange inflow")
        except (TypeError, ValueError):
            pass

        direction = str(signals.get("derivatives_bias") or "neutral").lower()
        flow = str(signals.get("flow_bias") or "neutral").lower()
        squeeze = str(signals.get("squeeze_risk") or "low").lower()
        if is_vi:
            outlook = {"bullish": "thiên về tăng", "bearish": "thiên về giảm", "neutral": "trung lập"}.get(
                direction if direction != "neutral" else flow,
                "trung lập",
            )
            risk = {"high": "rủi ro tập trung cao", "medium": "mức độ tập trung đang tăng", "low": "rủi ro tập trung thấp"}.get(
                squeeze,
                "rủi ro chưa xác định",
            )
            if parts:
                return f"Tóm tắt dữ liệu phái sinh: {', '.join(parts)}; xu hướng {outlook}; {risk}."
            return f"Tóm tắt dữ liệu phái sinh: xu hướng {outlook}; {risk}."

        outlook = {"bullish": "bullish", "bearish": "bearish", "neutral": "neutral"}.get(
            direction if direction != "neutral" else flow,
            "neutral",
        )
        risk = {"high": "high crowding risk", "medium": "rising crowding", "low": "low crowding risk"}.get(
            squeeze,
            "unknown crowding risk",
        )
        if parts:
            return f"Derivative data summary: {', '.join(parts)}; {outlook} bias; {risk}."
        return f"Derivative data summary: {outlook} bias; {risk}."
    ...
    def _calculate_fundamental_score(self, fundamental: Dict, market: str) -> float:
        ...
        if market not in ("USStock", "VNStock") or not fundamental:
            return 50.0
    ...
    def _calculate_crypto_factor_score(
        ...
        def add(name: str, value: float, reason: str, reason_vi: str = ""):
            nonlocal score
            score += float(value)
            breakdown.append({
                "factor": name,
                "score": round(float(value), 2),
                "reason": reason_vi if is_vi and reason_vi else reason,
            })
            ...
            if funding_rate is not None and oi_change is not None:
                ...
                if fr > 0 and oi > 3:
                    add(
                        ...
                        "Phí tài trợ dương cùng OI tăng củng cố động lượng tăng.",
                    )
                elif fr < 0 and oi > 3:
                    add(
                        "funding_oi", -18,
                        "Negative funding with rising open interest strengthens short momentum.",
                        "Phí tài trợ âm cùng OI tăng củng cố động lượng giảm.",
                    )
        except Exception:
            pass
            ...
            if exchange_netflow is not None:
                ...
                if enf < 0:
                    add(
                        ...
                        "Dòng tiền ròng rút khỏi sàn cho thấy nguồn cung bán tức thời giảm.",
                    )
                elif enf > 0:
                    add(
                        "exchange_netflow", -16,
                        "Exchange net inflow indicates higher potential sell pressure.",
                        "Dòng tiền ròng vào sàn cho thấy áp lực bán tiềm ẩn cao hơn.",
                    )
        except Exception:
            pass
        ...
        try:
            if stablecoin_netflow is not None:
                ...
                if stf > 0:
                    add(
                        "stablecoin_netflow", 12,
                        "Stablecoin net inflow indicates stronger potential buying power.",
                        "Dòng tiền ròng stablecoin vào thị trường cho thấy sức mua tiềm năng mạnh hơn.",
                    )
                elif stf < 0:
                    add(
                        "stablecoin_netflow", -12,
                        "Stablecoin net outflow indicates weaker marginal buying power.",
                        "Dòng tiền ròng stablecoin rút ra cho thấy sức mua biên yếu hơn.",
                    )
        except Exception:
            pass
            ...
            if long_short_ratio is not None:
                ...
                if lsr > 1.6:
                    add(
                        ...
                        "Tỷ lệ long/short quá cao cho thấy vị thế mua đang tập trung quá mức.",
                    )
                elif lsr < 0.75:
                    add(
                        "long_short_ratio", 8,
                        "Deep short positioning creates potential for a short squeeze.",
                        "Vị thế bán sâu tạo ra khả năng xảy ra short squeeze.",
                    )
        except Exception:
            pass
        ...
        try:
            if volume_change is not None and change_24h is not None:
                ...
                if vol > 15 and chg > 0:
                    add(
                        "volume_price", 10,
                        "Rising price with strong volume confirms the trend.",
                        "Giá tăng cùng khối lượng mạnh xác nhận xu hướng.",
                    )
                elif vol > 15 and chg < 0:
                    add(
                        "volume_price", -10,
                        "Falling price with strong volume confirms bearish control.",
                        "Giá giảm cùng khối lượng mạnh xác nhận bên bán đang kiểm soát.",
                    )
                elif vol < -15 and abs(chg) > 3:
                    add(
                        "volume_price", -6 if chg > 0 else 6,
                        "Price movement with declining volume weakens trend confidence.",
                        "Giá biến động trong khi khối lượng suy giảm làm giảm độ tin cậy của xu hướng.",
                    )
        except Exception:
            pass
        ...
        if squeeze_risk == "high":
            add(
                "squeeze_risk", -8,
                "High derivatives crowding increases short-term volatility risk.",
                "Vị thế phái sinh tập trung cao làm tăng rủi ro biến động ngắn hạn.",
            )
        elif squeeze_risk == "medium":
            add(
                "squeeze_risk", -3,
                "Rising derivatives crowding calls for tighter risk control.",
                "Vị thế phái sinh tập trung tăng đòi hỏi kiểm soát rủi ro chặt chẽ hơn.",
            )

        summary = self._localized_crypto_factor_summary(crypto_factors, language)
        return {
