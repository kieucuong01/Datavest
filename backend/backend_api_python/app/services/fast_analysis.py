...
class FastAnalysisService(FastAnalysisScoringMixin):
    ...
    def _format_crypto_factor_prompt(self, crypto_factors: Dict[str, Any], language: str) -> str:
        ...
        if is_zh:
            return (
                ...
                f"- 因子摘要: {self._localized_crypto_factor_summary(crypto_factors, language) or '暂无'}"
            )
        ...
        return (
            ...
            f"- Factor summary: {self._localized_crypto_factor_summary(crypto_factors, language) or 'N/A'}"
        )
    ...
    def _build_analysis_prompt(self, data: Dict[str, Any], language: str) -> tuple:
        ...
        lang_map = {
            ...
            'vi-VN': (
                '⚠️ BẮT BUỘC: Bạn phải trả lời toàn bộ bằng tiếng Việt tự nhiên, đầy đủ dấu, '
                'bao gồm summary, key_reasons, risks và mọi trường văn bản. Không dùng văn xuôi tiếng Anh hoặc tiếng Trung. '
                + language_instruction('vi-VN', structured=True)
            ),
...
6. Provide your analysis now. Remember: all prices must be within 10% of ${current_price}."""
    ...
    def analyze(self, market: str, symbol: str, language: str = 'en-US', 
        ...
        fallback_copy = {
            ...
            "risk": "Đã xảy ra lỗi trong quá trình phân tích." if language == "vi-VN" else "Analysis error",
            "price_error": "Không lấy được giá hiện tại từ các nguồn dữ liệu." if language == "vi-VN" else "Failed to fetch current price from all sources",
        ...
        try:
            ...
            for tf in consensus_timeframes:
                ...
                objective = self._calculate_objective_score(d_tf, current_price_tf, language)
            ...
            if "1W" not in objective_by_tf:
                try:
                    ...
                    obj_1w = self._calculate_objective_score(d_1w, cp_1w, language)
            ...
            if "1H" not in objective_by_tf:
                try:
                    ...
                    obj_1h = self._calculate_objective_score(d_1h, cp_1h, language)
            ...
            if not current_price or current_price <= 0:
                result["error"] = fallback_copy["price_error"]
            ...
            default_struct = {
                "decision": "HOLD",
                "confidence": 50,
                "summary": fallback_copy["summary"],
                "entry_price": current_price,
                "stop_loss": current_price * 0.95,
                "take_profit": current_price * 1.05,
                "position_size_pct": 10,
                "timeframe": "medium",
                "key_reasons": [fallback_copy["reason"]],
                "risks": [fallback_copy["risk"]],
                "technical_score": 50,
            ...
            objective_score = self._calculate_objective_score(data, current_price, language)
            logger.info(
            ...
            if consensus_abs >= min_abs_override:
                ...
                vi_outlook = {"BUY": "tăng", "SELL": "giảm", "HOLD": "trung lập"}.get(final_decision, "trung lập")
                if is_vi:
                    level = "rất mạnh" if consensus_abs >= 70 else "rõ rệt" if consensus_abs >= 40 else "nhẹ"
                    bias = "tăng" if consensus_score > 0 else "giảm"
                    consensus_note = (
                        f"[Đồng thuận khách quan đa khung thời gian: điểm {consensus_score:.1f} "
                        f"({level}, thiên về {bias}); nhận định AI {vi_outlook}]"
                    )
                elif is_zh:
            ...
            if market == "Crypto" and not detailed_analysis.get("fundamental"):
                detailed_analysis["fundamental"] = crypto_factor_summary or self._localized_crypto_factor_summary(
                    data.get("crypto_factors") or {}, language
                )
            ...
            result["error"] = fallback_copy["exception"]
    ...
    def _validate_and_constrain(self, analysis: Dict, current_price: float, indicators: Dict = None,
                                 has_major_news: bool = False, has_macro_event: bool = False,
    ...
    def _validate_decision_against_indicators(self, analysis: Dict, indicators: Dict, confidence: int, 
        ...
        def append_validation_note(original_summary: str, *, overridden: bool) -> str:
            if language == "vi-VN":
                suffix = (
                    "Lưu ý: các chỉ báo kỹ thuật đang mâu thuẫn với quyết định; sự kiện lớn có thể làm xu hướng thay đổi."
                    if overridden
                    else "Lưu ý: các chỉ báo kỹ thuật đang mâu thuẫn với quyết định; nên chờ thêm xác nhận."
                )
                return f"{original_summary} [{suffix}]".strip()
            if language == "en-US":
                suffix = (
                    "Note: technical indicators conflict with the decision; a major event may change the trend."
                    if overridden
                    else "Note: technical indicators conflict with the decision; wait for further confirmation."
                )
                return f"{original_summary} [{suffix}]".strip()
            return f"{original_summary} [注意：技术指标显示{', '.join(conflicts)}，但重大事件可能改变趋势]".strip()
        ...
        if decision == "BUY":
            ...
            if conflicts:
                if allow_override:
                    ...
                    original_summary = analysis.get("summary", "")
                    analysis["summary"] = append_validation_note(original_summary, overridden=True)
                else:
                    ...
                    original_summary = analysis.get("summary", "")
                    analysis["summary"] = append_validation_note(original_summary, overridden=False)
