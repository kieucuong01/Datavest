...
USER_SCOPES = frozenset(ALL_SCOPES)

_USER_FUND_RISKS = [
    "A leaked token grants API access within its scopes until revoked; treat it as a secret.",
    "Paper simulations consume platform compute and job queue capacity.",
]

_SYSTEM_RISKS = [
    "Many concurrent agents increase load on DB connection pools, agent job workers, and background strategy threads.",
    "Burst traffic from automated clients can hit per-token rate limits, affecting other tenants on shared infrastructure.",
    "Audit logs record calls but cannot replace human review of agent behavior or strategy changes.",
    "Automated research output still requires human review before it is used for decisions.",
]
...
def get_token_policy(*, for_admin: bool = False) -> dict[str, Any]:
    ...
    return {
        ...
        "is_saas": saas,
        "allowed_scopes": sorted(USER_SCOPES),
...
def _parse_issue_body(body: dict[str, Any]) -> dict[str, Any]:
    name = (body.get("name") or "").strip() or f"agent-{int(datetime.utcnow().timestamp())}"
    raw_scopes = body.get("scopes")
    if isinstance(raw_scopes, str):
        requested_scopes = {part.strip().upper() for part in raw_scopes.split(",") if part.strip()}
    elif raw_scopes is None:
        requested_scopes = {"R"}
    else:
        requested_scopes = {str(part).strip().upper() for part in raw_scopes if str(part).strip()}
    unknown_scopes = requested_scopes - set(ALL_SCOPES)
    if unknown_scopes:
        raise TokenIssueError(f"Unknown scope in {sorted(unknown_scopes)}", code=400)
...
def issue_agent_token(user_id: int, body: dict[str, Any]) -> dict[str, Any]:
    ensure_agent_gateway_schema()
