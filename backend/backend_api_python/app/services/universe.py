...
def normalize_member(raw: dict, *, default_market: str = "") -> dict:
    ...
    try:
        ...
        raise UniverseError("universe.invalidMarket") from exc
    raw_symbol = normalize_symbol(raw.get("symbol"))
    if not raw_symbol:
        raise UniverseError("universe.invalidSymbol")
    try:
        symbol = canonicalize_supported_symbol(market, raw_symbol)
    except (TypeError, ValueError) as exc:
        raise UniverseError("universe.invalidSymbol") from exc
    if market == "Crypto":
...
class UniverseService:
        ...
        with get_db_connection() as db:
            ...
            cur.execute(
                ...
                WHERE (u.is_system = TRUE OR u.user_id = ?)
                  AND u.status NOT IN ('deprecated', 'archived')
            ...
            cur.close()
        items = [
            _serialize_universe(row)
            for row in rows
            if self._is_supported_universe(row)
        ]
        for item in items:
    ...
    def system_overview(self) -> dict:
        ...
        items = []
        for row in rows:
            if not self._is_supported_universe(row):
                continue
        ...
        return {
            "universes": items,
            "syncable_codes": [
                "sp500", "nasdaq100", "crypto_top100", "us_etf",
            ],
        }
    ...
    def resolve_members(self, user_id: int, universe_id: int, *, as_of: Any = None) -> list[dict]:
        ...
        return [
            member
            for row in rows
            for member in [self._serialize_supported_member(row)]
    ...
    @staticmethod
    def _serialize_supported_member(row: dict) -> dict | None:
        try:
    ...
    def candidate_members(
        ...
        *,
        ...
        deduped = {}
        for row in rows:
            member = self._serialize_supported_member(row)
            if member is None:
                continue
            key = (
                member["market"], member["symbol"], member["exchange_id"],
    ...
    def _get_visible_universe(db, user_id: int, universe_id: int, *, require_owner: bool = False) -> dict:
        ...
        row = cur.fetchone()
        cur.close()
        if not row or not UniverseService._is_supported_universe(row):
            raise UniverseError("universe.notFound", status_code=404)
        return row

    @staticmethod
    def _is_supported_universe(row: dict) -> bool:
        if str(row.get("status") or "").strip().lower() in {"deprecated", "archived"}:
            return False
        market = str(row.get("market") or "").strip()
        if market == "Mixed":
            return True
        try:
            normalize_supported_market(market)
        except (TypeError, ValueError):
            return False
        return True
