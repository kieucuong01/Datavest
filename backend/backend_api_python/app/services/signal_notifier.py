...
def _signal_payload_text(payload: Dict[str, Any]) -> str:
    ...
    lines = [
        f"DataVest Signal - {symbol} - {' '.join([action, side]).strip()}",
...
def _profile_test_text(payload: Dict[str, Any]) -> str:
    title = str((payload or {}).get("title") or "DataVest notification test")
...
class SignalNotifier:
    ...
    def _render_messages(self, payload: Dict[str, Any]) -> Dict[str, str]:
        ...
        telegram_lines = [
            f"<b>DataVest Signal</b> | <code>{html.escape(symbol or '-')}</code>",
        ...
        telegram_lines.append("<i>Generated by DataVest signal engine.</i>")
    ...
    <div style="max-width:680px;margin:0 auto;padding:28px 18px;font-family:Inter,Arial,sans-serif;">
      <div style="background:#141821;border:1px solid #252b38;border-radius:8px;overflow:hidden;box-shadow:0 20px 50px rgba(0,0,0,.35);">
        ...
        <div style="padding:14px 18px;color:#9ca3af;font-size:12px;border-top:1px solid #252b38;background:#0f131a;">
          Generated by DataVest signal engine. Review execution mode and exchange account before acting.
        </div>
      </div>
    </div>
  </body>
...
"""
    ...
    def _notify_webhook(
        ...
        *,
        ...
        URL hosts recognised automatically:
          ...
          - Generic     everything else (keeps DataVest's own schema)
        ...
        headers: Dict[str, str] = {
            ...
            "User-Agent": "DataVest/1.0 (+https://www.quantdinger.com)",
        }
        ...
        if signing_secret and dialect == 'feishu':
            ...
            # Original DataVest contract: body-bound HMAC-SHA256 hex
    ...
    def _notify_discord(self, *, url: str, payload: Dict[str, Any], fallback_text: str) -> Tuple[bool, str]:
        ...
        if str((payload or {}).get("event") or "") == "qd.profile_test":
            embed = {
                ...
                "color": 0x22C55E,
                "footer": {"text": "DataVest notification channel check"},
            }
        ...
        embed: Dict[str, Any] = {
            "title": f"DataVest Signal | {str(instrument.get('symbol') or '-')}",
            ...
            "footer": {"text": "Generated by DataVest signal engine"},
        }
        if payload.get("timestamp_iso"):
            embed["timestamp"] = str(payload.get("timestamp_iso") or "")
        if trace.get("pending_order_id"):
            embed["footer"] = {"text": f"Generated by DataVest signal engine | pending_order_id={int(trace.get('pending_order_id'))}"}
        return self._post_discord_embed(url=url, embed=embed, fallback_text=fallback_text)
    ...
    def send_profile_test_notifications(
        ...
        *,
        ...
        title = "DataVest kiểm tra thông báo" if vi else ("DataVest 通知测试" if zh else "DataVest notification test")
        plain = (
            "Đây là tin nhắn kiểm tra từ phần cài đặt thông báo trong hồ sơ DataVest. Nếu bạn nhận được tin này, kênh đã được cấu hình đúng."
            if vi
            else
            "这是一条来自 DataVest 个人中心「通知设置」的测试消息。若您收到本条消息，说明该渠道配置正确。"
            if zh
            else "This is a test message from DataVest profile notification settings. "
        ...
        notification_heading = "Thông báo DataVest" if vi else "DataVest Notification"
        html_body = f"""\
<!doctype html>
<html>
  <body style="margin:0;padding:0;background:#090b10;">
    <div style="max-width:640px;margin:0 auto;padding:28px 18px;font-family:Inter,Arial,sans-serif;">
      <div style="background:#141821;border:1px solid #252b38;border-radius:8px;overflow:hidden;">
        <div style="padding:22px 24px;background:linear-gradient(135deg, rgba(59,130,246,.20), rgba(34,197,94,.10));border-bottom:1px solid #252b38;">
          <div style="font-size:12px;color:#9ca3af;font-weight:700;text-transform:uppercase;">{html.escape(notification_heading)}</div>
...
"""
        telegram_tail = "Kiểm tra kênh thông báo DataVest đã thành công." if vi else "DataVest notification channel check passed."
        telegram_html = f"<b>{html.escape(title)}</b>\n\n{html.escape(plain)}\n\n<i>{html.escape(telegram_tail)}</i>"
