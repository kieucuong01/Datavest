...
"""
    ...
    def send_profile_test_notifications(
        ...
        *,
        ...
        title = "QuantDinger kiểm tra thông báo" if vi else ("QuantDinger 通知测试" if zh else "QuantDinger notification test")
        plain = (
            "Đây là tin nhắn kiểm tra từ phần cài đặt thông báo trong hồ sơ QuantDinger. Nếu bạn nhận được tin này, kênh đã được cấu hình đúng."
            if vi
            else
        ...
        notification_heading = "Thông báo QuantDinger" if vi else "QuantDinger Notification"
        html_body = f"""\
<!doctype html>
<html>
  <body style="margin:0;padding:0;background:#090b10;">
...
"""
        telegram_tail = "Kiểm tra kênh thông báo QuantDinger đã thành công." if vi else "QuantDinger notification channel check passed."
