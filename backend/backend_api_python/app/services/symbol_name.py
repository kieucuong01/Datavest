...
- Vietnamese stocks are resolved from the curated local symbol master.
- Crypto and gold use canonical symbol fallbacks.
"""
...
from app.utils.supported_markets import SUPPORTED_MARKETS, canonicalize_supported_symbol
...
_CACHEABLE_MARKETS = SUPPORTED_MARKETS
...
def persist_seed_name(market: str, symbol: str, name: str) -> None:
    ...
    supported public stock and crypto providers.
    ...
    - Skips when ``name`` is empty or simply echoes ``symbol`` (Crypto/Forex/
      unsupported instrument resolvers commonly fall back to returning the ticker, which is
      useless to cache).
...
def _normalize_symbol_for_market(market: str, symbol: str) -> str:
    ...
    if m == 'Forex':
        try:
            return canonicalize_supported_symbol(m, s)
        except ValueError:
            return ''
    return s
...
def resolve_symbol_name(market: str, symbol: str) -> Optional[str]:
    ...
    # Crypto/gold: fall back to returning the symbol itself. We
