...
class SkillDefinition:
    ...
    def localized_copy(self, language: str) -> tuple[str, str, str]:
        if (language or "").lower().replace("_", "-").startswith("vi"):
            vietnamese = _VI_SKILL_COPY.get(self.id)
            if vietnamese:
                label, description = vietnamese
                prompt = (
                    f"Hãy thực hiện tác vụ “{label}” cho {{symbol_label}}. "
                    f"{description}. Chỉ dùng dữ liệu có nguồn trong hệ thống; "
                    "nêu rõ dữ liệu còn thiếu, giả định, điều kiện vô hiệu và rủi ro."
                )
                return label, description, prompt
        return (
            self.label.pick(language),
            self.description.pick(language),
            self.prompt_template.pick(language),
        )

    def to_public(self, language: str) -> dict[str, Any]:
        label, description, prompt = self.localized_copy(language)
        return {
            "id": self.id,
    ...
    def prompt_line(self, language: str) -> str:
        label, description, _ = self.localized_copy(language)
...
_VI_SKILL_COPY: dict[str, tuple[str, str]] = {
    "market_diagnosis": ("Chẩn đoán thị trường", "Phân tích xu hướng, khối lượng, vùng giá, dòng vốn và rủi ro"),
    "entity_discovery": ("Nhận diện tài sản", "Xác định doanh nghiệp, mã giao dịch, bí danh và tài sản đại diện từ ngôn ngữ tự nhiên"),
    "news_research": ("Nghiên cứu tin tức", "Tổng hợp tin về doanh nghiệp, tài sản, vĩ mô và ngành kèm nguồn dẫn"),
    "macro_economic_data": ("Dữ liệu kinh tế vĩ mô", "Tra cứu việc làm, lạm phát, lãi suất, tăng trưởng và các sự kiện tác động thị trường"),
    "market_data_lookup": ("Tra cứu dữ liệu thị trường", "Tra cứu giá, nến, khối lượng và dữ liệu theo khung thời gian"),
    "chart_review": ("Phân tích biểu đồ", "Đánh giá điểm vào, dừng lỗ và điều kiện vô hiệu từ biểu đồ được cung cấp"),
    "market_scanner": ("Quét cơ hội thị trường", "Lọc cơ hội theo biến động, khối lượng, độ biến động giá và cấu trúc xu hướng"),
    "symbol_resolver": ("Xác định mã tài sản", "Xác định thị trường, mã, bí danh và ngữ cảnh dữ liệu từ yêu cầu tự nhiên"),
    "data_source_doctor": ("Kiểm tra nguồn dữ liệu", "Kiểm tra mức sẵn sàng của giá, nến, khối lượng, tài chính và tin tức"),
    "indicator_authoring": ("Soạn chỉ báo", "Soạn chỉ báo đúng hợp đồng của Chỉ báo IDE"),
    "indicator_research": ("Nghiên cứu chỉ báo", "Tạo mã chỉ báo, tham số và dấu hiệu trực quan chỉ dùng trên biểu đồ"),
    "indicator_validation": ("Kiểm tra chỉ báo", "Kiểm tra cú pháp, kích thước, đầu ra và giới hạn an toàn trước khi lưu"),
    "backtest_runner": ("Chạy kiểm thử chiến lược", "Chạy kiểm thử lịch sử và giải thích lợi nhuận, sụt giảm, tỷ lệ thắng, giao dịch và thiên lệch"),
    "script_strategy": ("Nghiên cứu chiến lược", "Tạo bản nháp Strategy API V2 để kiểm thử và nghiên cứu"),
    "strategy_requirements_interview": ("Thu thập yêu cầu chiến lược", "Làm rõ khung thời gian, thị trường, rủi ro, quy mô vị thế và điều kiện thực thi trước khi viết mã"),
    "script_template_plan": ("Mẫu chiến lược", "Đề xuất tham số cho chiến lược lưới, xu hướng, tích lũy định kỳ và mã tùy chỉnh kèm kiểm soát rủi ro"),
    "scheduled_analysis": ("Phân tích định kỳ", "Theo dõi tài sản theo lịch và lưu kết quả phân tích"),
    "portfolio_monitoring": ("Theo dõi danh mục", "Theo dõi vị thế mô phỏng, lãi lỗ, điểm dừng, mức phơi nhiễm và cảnh báo"),
    "watchlist_manage": ("Thêm vào danh sách theo dõi", "Thêm tài sản đã xác nhận vào danh sách theo dõi"),
    "agent_token_advisor": ("Tư vấn mã truy cập tác nhân", "Thiết kế quyền tối thiểu cho tác nhân AI và công cụ tích hợp bên ngoài"),
    "mcp_integration": ("Tích hợp MCP", "Cấu hình máy chủ MCP, kết nối ứng dụng AI bên ngoài và kiểm tra công cụ"),
    "job_monitor": ("Theo dõi tác vụ", "Theo dõi trạng thái, tiến độ, kết quả và lỗi của tác vụ nền"),
    "notification_setup": ("Cấu hình thông báo", "Cấu hình thông báo trong ứng dụng, email, SMS, Telegram và webhook"),
    "opportunity_radar": ("Radar cơ hội", "Tìm các tín hiệu cơ hội trong 24 giờ"),
    "audit_and_compliance": ("Kiểm tra và tuân thủ", "Rà soát lệnh gọi tác nhân, thao tác người dùng, thay đổi chiến lược và nhật ký nhạy cảm"),
    "knowledge_memory": ("Ghi nhớ người dùng", "Lưu sở thích, giới hạn giao dịch, thị trường quan tâm và phong cách chiến lược"),
    "system_release_doctor": ("Kiểm tra bản phát hành", "Kiểm tra frontend, backend, Docker, migration và các luồng quan trọng"),
}
...
_SKILLS: tuple[SkillDefinition, ...] = (
    ...
    SkillDefinition(
        ...
        description=_s("检查 LLM、数据源和通知配置", "Check LLM, data source, and notifications"),
        prompt_template=_s(
            "请帮我检查 DataVest 当前部署是否完整：LLM/API Key、数据源、网络代理、通知配置。请按问题现象、可能原因、配置入口、验证方式输出。",
            "Check whether my DataVest deployment is complete: LLM/API key, data source, proxy, and notifications. Output symptoms, causes, where to configure, and how to verify.",
        ),
...
_EXTRA_SKILLS: tuple[SkillDefinition, ...] = (
    ...
    SkillDefinition(
        ...
        prompt_template=_s(
            ...
            "Resolve the true research object from my question. First classify whether it is a US stock, Vietnam stock, crypto asset, gold/XAU, or a macro event. If it is outside the supported US/Vietnam stocks, crypto, and gold scope, explain that it is unavailable instead of inventing a provider or proxy.",
        ),
    ...
    SkillDefinition(
        ...
        prompt_template=_s(
            ...
            "Retrieve macro data or calendar events relevant to my question. State availability, source, release time, actual/forecast/previous values; if fields are missing, say what is missing and provide an impact framework for US/Vietnam equities, gold/XAU, and crypto. Do not add unsupported tradable markets.",
        ),
    ...
    SkillDefinition(
        ...
        system_instruction="Be conservative. Keep recommendations within research, backtest, and paper boundaries.",
    ...
    SkillDefinition(
        ...
        prompt_template=_s(
            "请根据我的外部 AI/MCP 使用场景，设计最小权限 Agent Token。请说明需要的 R/W/B/N 范围、市场白名单、标的白名单、过期时间和安全注意事项。",
            "Design a least-privilege Agent Token for my external AI/MCP use case. Cover R/W/B/N scopes, market allowlist, instrument allowlist, expiry, and safety notes.",
        ),
        system_instruction="Use only R/W/B/N scopes and keep portfolio workflows paper-only.",
...
def _all_skill_entries(include_disabled: bool = False) -> list[tuple[SkillDefinition, dict[str, Any]]]:
    ...
    for skill in (*_SKILLS, *_EXTRA_SKILLS):
        if skill.id in {"trade_plan", "risk_guard", "setup_doctor"}:
            continue
...
def render_prompt_template(skill: SkillDefinition, language: str, context: dict[str, Any] | None = None) -> str:
    ...
    if symbol and market:
        ...
        symbol_label = "当前标的" if language_key.startswith("zh") else ("tài sản hiện tại" if language_key.startswith("vi") else "the current symbol")
    _, _, prompt_template = skill.localized_copy(language)
    try:
        return prompt_template.format(symbol_label=symbol_label)
