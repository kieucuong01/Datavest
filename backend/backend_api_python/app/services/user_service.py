...
_CONSTRAINT_NAME_RE = re.compile(r'\b(qd_[A-Za-z0-9_]+_key)\b')
...
class UserService:
    ...
    ROLE_PERMISSIONS = {
        ...
        'admin': ['dashboard', 'view', 'indicator', 'backtest', 'strategy', 'portfolio', 'settings', 'user_manage'],
    }
    ...
    def get_user_by_id(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            with get_db_connection() as db:
                ...
                cur.execute(
                    """
                    SELECT id, username, email, nickname, avatar, status, role, timezone,
                           COALESCE(
    ...
    @staticmethod
    def _database_error_summary(error: Exception) -> str:
        constraint = getattr(getattr(error, 'diag', None), 'constraint_name', None)
        if not constraint:
            match = _CONSTRAINT_NAME_RE.search(str(error))
            constraint = match.group(1) if match else None
        summary = type(error).__name__
        return f"{summary} (constraint={constraint})" if constraint else summary

    def _seed_new_user_defaults(self, db, user_id: int, password: str) -> None:
        if password and not self._is_bootstrap_default_plain_password(password):
            self.mark_password_changed(user_id)
        try:
            _seed_default_watchlist(db, user_id)
        except Exception as seed_err:
            logger.warning("Default watchlist seed failed for bootstrap administrator: %s", type(seed_err).__name__)
        try:
            from app.services.builtin_indicators import seed_builtin_indicators_for_new_user
            ...
            logger.warning(
                "Builtin indicator seed failed for bootstrap administrator: %s",
                type(indicator_err).__name__,
            )

    def _insert_bootstrap_admin_if_database_empty(
        self,
        db,
        *,
        username: str,
        ...
        try:
            cur.execute(
                ...
                SELECT ?, ?, ?, ?, ?, ?, ?, NOW(), NOW()
                WHERE NOT EXISTS (SELECT 1 FROM qd_users)
    ...
    def _create_bootstrap_admin_if_database_empty(self) -> Optional[int]:
        from app.config.settings import Config

        admin_user = str(os.getenv('ADMIN_USER', Config.ADMIN_USER) or '').strip()
        admin_password = str(os.getenv('ADMIN_PASSWORD', Config.ADMIN_PASSWORD) or '')
        admin_email = str(os.getenv('ADMIN_EMAIL', 'admin@example.com') or '').strip() or None
        if len(admin_user) < 3 or len(admin_user) > 50:
            raise ValueError("Configured admin username must be 3-50 characters")
        if len(admin_password) < 6:
            raise ValueError("Configured admin password must be at least 6 characters")

        password_hash = self.hash_password(admin_password)
        with get_db_connection() as db:
            user_id = self._insert_bootstrap_admin_if_database_empty(
                db,
                username=admin_user,
            ...
            if user_id is None:
                return None
            self._seed_new_user_defaults(db, user_id, admin_password)
            logger.info("Created bootstrap administrator")
            return user_id
    ...
    def ensure_admin_exists(self):
        ...
        self.ensure_password_changed_column()
        try:
            self._create_bootstrap_admin_if_database_empty()
            self.sync_bootstrap_admin_credentials_from_env()
            self.sync_admin_email_from_config(overwrite_existing=False)
        except Exception as e:
            summary = self._database_error_summary(e)
            logger.error("ensure_admin_exists failed: %s", summary)
            raise RuntimeError(f"ensure_admin_exists failed: {summary}") from e
