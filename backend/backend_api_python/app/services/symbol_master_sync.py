...
STATIC_MARKET_ROWS = [
    ...
    SymbolMasterRow("VNStock", "VNINDEX", "VN-Index", "HOSE", "VND", asset_class="index"),
    SymbolMasterRow("VNStock", "VN30", "VN30 Index", "HOSE", "VND", asset_class="index"),
    SymbolMasterRow("VNStock", "FPT", "FPT Corporation", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "VCB", "Vietcombank", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "HPG", "Hoa Phat Group", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "VIC", "Vingroup", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "VHM", "Vinhomes", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "VNM", "Vinamilk", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "SSI", "SSI Securities", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "MWG", "Mobile World", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "VRE", "Vincom Retail", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "MSN", "Masan Group", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "GAS", "PV GAS", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "BID", "BIDV", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "CTG", "VietinBank", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "TCB", "Techcombank", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "ACB", "ACB", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "MBB", "MB Bank", "HOSE", "VND"),
    SymbolMasterRow("VNStock", "VPB", "VPBank", "HOSE", "VND"),
]
...
def fetch_crypto_symbols_with_diagnostics():
    ...
    for exchange_id in PUBLIC_KLINE_EXCHANGE_IDS:
        for market_type in ("spot", "swap"):
            ...
            try:
                ccxt_id, options = resolve_public_ccxt_market(exchange_id, market_type)
                config = {
...
def fetch_forex_symbols() -> List[SymbolMasterRow]:
    """Return Gold/XAU only in the Forex provider namespace."""
...
FETCHERS = {
    ...
    "VNStock": fetch_vn_stock_symbols,
...
def upsert_symbol_master(rows: Sequence[SymbolMasterRow]) -> int:
    ...
    for row in rows or []:
        try:
            market = normalize_supported_market(row.market)
            symbol = canonicalize_supported_symbol(market, row.symbol)
        except (TypeError, ValueError):
            continue
        if market != row.market or symbol != row.symbol:
            row = SymbolMasterRow(
                market=market,
                symbol=symbol,
                name=row.name,
