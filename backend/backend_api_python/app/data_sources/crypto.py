...
class CryptoDataSource(BaseDataSource):
    ...
    def __init__(self):
        # Public chart/backtest data may fail over between public providers.
        self._allow_public_fallback = True
        self._public_market_type = "spot"
    ...
    def for_public_market(
        ...
        """Return an uncredentialed source for public market data and research."""
        return cls._for_public_exchange(market_type, preferred_exchange_id, allow_fallback=True)

    @classmethod
    def _for_public_exchange(
        cls,
        market_type: str,
        preferred_exchange_id: str,
        *,
        ...
        if exchange_id not in PUBLIC_KLINE_EXCHANGE_IDS:
            exchange_id = "binance"
        cache_key = f"{exchange_id}|{mt}|{int(allow_fallback)}"
        cached = _PUBLIC_MARKET_INSTANCES.get(cache_key)
        if cached is not None:
            return cached
        ccxt_id, options = resolve_public_ccxt_market(exchange_id, mt)
        inst = object.__new__(cls)
        inst._allow_public_fallback = allow_fallback
        inst._public_market_type = mt
    ...
    def _symbol_for_public_market(self, symbol: str) -> str:
        ...
        if not normalized:
            return symbol
        mt = getattr(self, "_public_market_type", "") or "spot"
        if mt != "swap":
            return normalized
        if ":" in normalized:
            return normalized
    ...
    def get_ticker(self, symbol: str) -> Dict[str, Any]:
        ...
        fallback_market_type = str(
            getattr(self, "_public_market_type", "") or "spot"
        ).strip().lower()
        if (
            ...
            preferred_ticker = type(self)._for_public_exchange(
        ...
        normalized = self._symbol_for_public_market(symbol)
        ...
        if bool(getattr(self, "_allow_public_fallback", False)):
            for exchange_id in PUBLIC_KLINE_FALLBACK_IDS:
                if exchange_id == current_exchange:
                    continue
                fallback_ticker = type(self)._for_public_exchange(
                    fallback_market_type,
                    exchange_id,
                    allow_fallback=False,
                ).get_ticker(symbol)
    ...
    def get_kline(
        ...
        if (
            ...
            preferred_rows = type(self)._for_public_exchange(
                fallback_market_type,
                preferred_exchange,
                allow_fallback=False,
        ...
        try:
            ...
            symbol_pair = self._symbol_for_public_market(symbol)
        ...
        if not klines and bool(getattr(self, "_allow_public_fallback", False)):
            ...
            for exchange_id in PUBLIC_KLINE_FALLBACK_IDS:
                if exchange_id == current_exchange:
                    continue
                try:
                    fallback_source = type(self)._for_public_exchange(
                        fallback_market_type,
                        exchange_id,
                        allow_fallback=False,
                    )
