...
class DataSourceFactory:
    ...
    _CANONICAL_MARKETS = SUPPORTED_MARKET_ORDER
    ...
    def normalize_market(cls, market: str) -> str:
        """Normalize a market or reject it outside the DataVest scope."""
        return normalize_supported_market(market)
    ...
    def get_source(cls, market: str) -> BaseDataSource:
        ...
        Args:
            market: 市场类型 (USStock, VNStock, Crypto, Forex/XAU)
    ...
    def get_data_source(cls, name: str) -> BaseDataSource:
        ...
        if key in ("forex", "fx", "gold", "xau"):
            return cls.get_source("Forex")
        if key in ("usstock", "us_stocks", "stock", "stocks"):
            return cls.get_source("USStock")
        if key in ("vn", "vietnam", "vnstock", "vietnamstock"):
            return cls.get_source("VNStock")
        return cls.get_source(name)
    ...
    def _resolve_source(
        ...
        *,
        ...
        """Pick a read-only data source with optional public-provider preference."""
        ...
        if market == "Crypto" and ex:
            ...
            return CryptoDataSource.for_public_market(mt or "spot", preferred_exchange_id=ex)
