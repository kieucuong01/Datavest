...
def normalize_forex_pair_symbol(symbol: str) -> str:
    """Normalize the only supported Forex-namespace instrument: Gold/XAU."""
    if not symbol:
        raise ValueError("Empty gold symbol")
    s = str(symbol).strip().upper().replace(" ", "").replace("-", "")
    s = s.replace("/", "")
    if s in {"XAU", "XAUUSD", "GOLD"}:
        return "XAUUSD"
    raise ValueError("Only Gold/XAU is supported in the Forex provider namespace")
...
def _td_forex_symbol(symbol: str) -> str:
    """Convert the canonical XAU symbol to Twelve Data format."""
    s = normalize_forex_pair_symbol(symbol)
    ...
    def get_kline(
        ...
        for fetcher in (
            ...
            try:
                ...
                if bars:
                    self.last_kline_provider = {
                        "_get_kline_twelvedata": "twelvedata",
                        "_get_kline_tiingo": "tiingo",
                        "_get_kline_yfinance": "yfinance",
                    }.get(fetcher.__name__, fetcher.__name__)
