...
def _request_lang(default: str = "en-US") -> str:
    return detect_request_language(request, default=default)


def _is_zh_lang(lang: str | None) -> bool:
    return str(lang or "en-US").strip().lower().startswith("zh")


def _indicator_ai_text(key: str, lang: str = "en-US") -> str:
    texts = ({
        "prompt_required": "Vui lòng nhập yêu cầu cho AI.",
    } if str(lang or "").lower().startswith("vi") else {
        "prompt_required": "Prompt cannot be empty",
    })
    return texts.get(key, key)


def _indicator_hint_to_text(hint_code: str, params: Dict[str, Any] | None = None, lang: str = "en-US") -> str:
    params = params or {}
    if str(lang or "").lower().startswith("vi"):
        if hint_code == "DECLARED_PARAMS_NOT_READ_VIA_PARAMS_GET":
            joined = ", ".join(params.get("names") or []) or "các tham số"
            return f"Các tham số đã khai báo chưa được đọc bằng params.get(...): {joined}."
        if hint_code == "PARAM_DEFAULT_MISMATCH":
            items = params.get("items") or []
            detail = "; ".join(
                f"{item.get('name')}: @param={item.get('declared')}, giá trị dự phòng params.get={item.get('fallback')}"
                for item in items
            ) or "giá trị mặc định của tham số"
            return f"Giá trị mặc định của tham số không khớp: {detail}. Giá trị trong # @param phải trùng chính xác với giá trị dự phòng của params.get(...)."
        if hint_code == "SIGNAL_MARKERS_USE_WHERE_NONE":
            return "Dấu hiệu tín hiệu đang dùng where(..., None).tolist(); nên dùng danh sách None tường minh để tránh lỗi hiển thị NaN."
        if hint_code == "MISSING_OUTPUT":
            return "Thiếu từ điển output."
        if hint_code == "EXECUTION_COLUMNS_IGNORED_FOR_INDICATOR":
            return "Các cột thực thi bị bỏ qua trong chỉ báo biểu đồ. Hãy chuyển ý tưởng này sang Strategy API V2 trước khi kiểm thử chiến lược."
        if hint_code == "STRATEGY_ANNOTATIONS_IGNORED_FOR_INDICATOR":
            return "Không được dùng chú thích # @strategy trong chỉ báo biểu đồ. Hãy đặt quy tắc rủi ro, quy mô vị thế, khung thời gian và thực thi trong mã Strategy API V2."
        if hint_code == "MISSING_DF_COPY":
            return "Thiếu dòng df = df.copy()."
        if hint_code == "MISSING_INDICATOR_NAME":
            return "Thiếu my_indicator_name."
        if hint_code == "MISSING_INDICATOR_DESCRIPTION":
            return "Thiếu my_indicator_description."
        if hint_code == "NDARRAY_PANDAS_METHOD_MISUSE":
            symbol = params.get("symbol") or "ndarray"
            method = params.get("method") or "?"
            return f"Đang gọi phương thức pandas trên mảng numpy: {symbol}.{method}(...). Hãy bọc bằng pd.Series(arr, index=df.index) hoặc viết lại bằng phương thức pandas tương ứng."
        if hint_code == "HELPER_RETURNS_NDARRAY":
            names = params.get("names_str") or ", ".join(params.get("names") or []) or "hàm hỗ trợ"
            return f"Các hàm hỗ trợ trả về mảng numpy: {names}. Hãy trả về Series để dùng an toàn với rolling/fillna/shift/ewm/iloc."
        if hint_code == "RUNTIME_ERROR_ON_VERIFY":
            error_type = params.get("error_type") or "RuntimeError"
            detail = params.get("detail") or ""
            return f"Lần chạy thử trong sandbox phát sinh {error_type}: {detail}."
        if hint_code == "FUTURE_DATA_LEAK":
            snippet = params.get("snippet") or "?"
            return f"Phát hiện đọc dữ liệu tương lai: {snippet}. Kiểm thử đang dùng các nến chưa xảy ra; hãy chỉ tham chiếu dữ liệu quá khứ."
        return f"Phát hiện lưu ý trong mã: {hint_code}"
    if hint_code == "DECLARED_PARAMS_NOT_READ_VIA_PARAMS_GET":
    ...
    if hint_code == "EXECUTION_COLUMNS_IGNORED_FOR_INDICATOR":
        return "Execution columns are ignored in chart indicators. Convert this idea to Strategy API V2 before backtesting."
    if hint_code == "STRATEGY_ANNOTATIONS_IGNORED_FOR_INDICATOR":
...
def _indicator_human_summary(
    ...
    if str(lang or "").lower().startswith("vi"):
        if auto_fix_applied and auto_fix_succeeded:
            title = "AI đã tự sửa mã chỉ báo và trả về phiên bản ổn định hơn"
        elif auto_fix_applied:
            title = "AI đã thử tự sửa mã nhưng vẫn còn một số vấn đề"
        else:
            title = "AI đã tạo mã chỉ báo và mã đã vượt qua quy trình kiểm tra hiện tại"
        returned_text = (
            "Mã trả về là phiên bản đã được tự động sửa."
            if returned_candidate == "repaired"
            else "Mã trả về là phiên bản được tạo ban đầu."
        )
    else:
        if auto_fix_applied and auto_fix_succeeded:
...
def get_indicators():
    ...
    try:
        ...
        with get_db_connection() as db:
            cur = db.cursor()
            # Get the user's own indicators, including community forks.
            cur.execute(
                """
                SELECT
                  id, user_id, is_buy, end_time, name, code, description,
                  publish_to_community, preview_image,
...
def save_indicator():
    ...
    try:
        ...
        try:
            ...
            price = 0.0
        if publish_to_community and (
            str(data.get("pricingType") or "free").strip().lower() != "free"
            or price > 0
            or bool(data.get("vipFree") or data.get("codeHidden") or data.get("hideCode"))
        ):
            return jsonify({
                "code": 0,
                "msg": "community_publication_must_be_free_and_visible",
                "data": None,
            }), 400
            ...
            if indicator_id and indicator_id > 0:
                if publish_to_community:
                    ...
                    if not was_published:
                        cur.execute(
                            ...
                            SET name = ?, code = ?, description = ?,
                                publish_to_community = ?, preview_image = ?, asset_type = ?,
                            ...
                            """,
                            (name, code, description, publish_to_community, preview_image, asset_type,
                    ...
                    cur.execute(
                        ...
                        """,
                        (name, code, description, publish_to_community, preview_image, asset_type, now, indicator_id, user_id),
                    )
            else:
                review_status = None
                if publish_to_community:
                ...
                cur.execute(
                    ...
                    INSERT INTO qd_indicator_codes
                      (user_id, is_buy, end_time, name, code, description,
                       publish_to_community, preview_image, asset_type, review_status,
                       createtime, updatetime, created_at, updated_at)
                    VALUES (?, 0, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
                    """,
                    (user_id, name, code, description, publish_to_community, preview_image, asset_type, review_status, now, now),
                )
...
def delete_indicator():
    ...
    try:
        ...
        with get_db_connection() as db:
            ...
            was_published = int(row.get("publish_to_community") or 0) == 1
            if was_published:
                # Preserve the public lineage record; user deletion becomes unpublish.
                cur.execute(
                    """
                    UPDATE qd_indicator_codes
        ...
        return jsonify({"code": 1, "msg": "success", "data": {"unpublished": was_published}})
    except Exception as e:
