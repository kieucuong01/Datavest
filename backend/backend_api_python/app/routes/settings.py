    ...
    'brand': {
        ...
        'items': [
            ...
            },
            {
                ...
                'type': 'text',
                'required': False,
                'description': 'Public URL to a wide logo for the light theme. Recommended size 240x60 px (PNG / SVG / WebP, ~4:1 aspect ratio, transparent background). Leave empty to use the bundled DataVest logo.'
            },
            {
                ...
                'type': 'text',
                'required': False,
                'description': 'Public URL to a wide logo for the dark theme. Recommended size 240x60 px (PNG / SVG / WebP, ~4:1 aspect ratio, transparent background). Leave empty to use the bundled DataVest logo.'
            },
            {
        ...
        'items': [
            {
                ...
                'description': 'Optional narrowing of supported product markets (USStock, VNStock, Crypto, Forex/XAU). Retired markets cannot be re-enabled.'
            },
        ]
    },
    ...
    'data_source': {
        ...
        'items': [
            ...
            },
            {
                ...
                'link_text': 'settings.link.getApiKey',
                'description': 'Twelve Data API key for supported US/Vietnam stock or XAU/USD K-lines (free 800 credits/day)'
            },
            {
                'key': 'ADANOS_API_KEY',
    ...
    'agent': {
        ...
        'items': [
            # Agent Gateway (/api/agent/v1) deployment knobs
            {
                ...
                'default': '',
                'description': 'Set to "saas" on multi-tenant hosted instances. Users self-manage R/W/B/N Agent Tokens in Profile.'
            },
            {
...
def _requested_unknown_settings(data) -> list[str]:
    unknown = set()
    for group_name, group_values in (data or {}).items():
        if not isinstance(group_values, dict):
            continue
        group = CONFIG_SCHEMA.get(group_name)
        if not group:
            unknown.update(str(key) for key in group_values)
            continue
        allowed = {str(item.get("key")) for item in group.get("items", [])}
        unknown.update(str(key) for key in group_values if str(key) not in allowed)
...
def save_settings():
    ...
    try:
        ...
        unknown_keys = _requested_unknown_settings(data)
        if unknown_keys:
            return jsonify({
                'code': 0,
                'msg': 'forbidden_setting',
                'data': {'keys': unknown_keys},
            }), 400
