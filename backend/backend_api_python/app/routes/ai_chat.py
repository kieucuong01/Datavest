...
from app.utils.language import detect_request_language, language_instruction, normalize_product_language
...
def _input_error(language: str, vietnamese: str, english: str) -> str:
    """Return a user-facing validation message in the requested product language."""
    return vietnamese if normalize_product_language(language) == "vi-VN" else english
...
def _agent_response_language_name(language: str) -> str:
    normalized = normalize_product_language(language).lower()
...
_TICKER_DISCOVERY_RE = (
    re.compile(r"\b(?:NASDAQ|NYSE|AMEX|NYSEARCA|OTC|HOSE|HNX|UPCOM)\s*[:：]\s*([A-Z0-9.]{1,8})\b", re.I),
...
def _local_symbol_rows_for_term(term: str, per_market_limit: int = 4) -> list[dict]:
    markets = ("USStock", "VNStock", "Crypto", "Forex")
    rows: list[dict] = []
...
def _build_research_context(context: dict, has_image: bool = False) -> dict:
    ...
    language = normalize_product_language(context.get("language"))
...
def _parse_llm_json(text: str, language: str = "en-US") -> dict:
    try:
    ...
    if match:
        try:
            ...
            pass
    is_vi = normalize_product_language(language) == "vi-VN"
    return {
        "answer": text or ("Mô hình không trả về nội dung câu trả lời." if is_vi else "The model returned an empty response."),
        "summary": "Phản hồi Copilot" if is_vi else "Copilot response",
        "confidence": 50,
        "actions": [],
...
def ai_skill_install():
    ...
    language = detect_request_language(request, data)
    try:
...
def agent_intent():
    ...
    if not message and not attachments:
        return jsonify({
            "code": 0,
            "msg": _input_error(language, "Vui lòng nhập nội dung cần trao đổi", "Missing message"),
            "data": None,
        }), 400
...
def save_user_memory():
    ...
    if not title or not content:
        return jsonify({
            "code": 0,
            "msg": _input_error(language, "Tiêu đề và nội dung là bắt buộc", "title and content are required"),
            "data": None,
        }), 400
    with get_db_connection() as db:
        cur = db.cursor()
...
def chat_message():
    ...
    try:
        with get_db_connection() as db:
            ...
            parsed = _parse_llm_json(raw, language)
            answer = str(parsed.get("answer") or raw or "").strip()
            if not answer:
                answer = "Mô hình không trả về câu trả lời có thể sử dụng." if language == "vi-VN" else "The model did not return a usable answer."
        ...
        message = "Không thể tạo câu trả lời AI. Vui lòng thử lại sau." if language == "vi-VN" else "Unable to generate an AI response. Please try again."
        return jsonify({"code": 0, "msg": message, "data": None}), 500
...
def _stream_llm_with_recovery(llm_messages: list[dict], temperature: float = 0.35, language: str = "en-US"):
    ...
    try:
        ...
        if finish_reason == "length" and has_partial:
            yield "warning", {
                "code": "output_limit",
                "msg": (
                    "Câu trả lời đã chạm giới hạn độ dài và có thể chưa đầy đủ."
                    if normalize_product_language(language) == "vi-VN"
                    else "The response reached the configured output limit and may be incomplete."
                ),
    ...
    def generate():
        ...
        try:
            with get_db_connection() as db:
                ...
                # Mark the request as accepted before provider work so clients
                ...
                answer = "".join(chunks).strip() or (
                    "Mô hình không trả về câu trả lời có thể sử dụng."
                    if language == "vi-VN"
                    else "The model did not return a usable answer."
                )
                assistant_id = _insert_message(
                    cur,
                    session_id=sid,
            ...
            yield _sse("error", {
                "msg": (
                    "Không thể tạo câu trả lời AI. Vui lòng thử lại sau."
                    if language == "vi-VN"
                    else "Unable to generate an AI response. Please try again."
                ),
                "session_id": sid,
...
def save_local_chat_message():
    ...
    if not content and report:
        ...
        title = "Báo cáo phân tích" if language == "vi-VN" else "Analysis report"
        content = f"{title}: {market}:{symbol}".strip(":")
    if not content and report_error:
        title = "Phân tích không thành công" if language == "vi-VN" else "Analysis failed"
        content = f"{title}: {report_error}"
    if not content:
        return jsonify({
            "code": 0,
            "msg": _input_error(language, "Vui lòng nhập nội dung tin nhắn", "Missing message content"),
            "data": None,
        }), 400
