...
def get_market_types():
    ...
    rules, primarily the ``ENABLED_MARKETS`` whitelist. Retired market flags
    cannot re-enable a market. The radar
    ...
    """
    desired_order = list(SUPPORTED_MARKET_ORDER)
    order_rank = {v: i for i, v in enumerate(desired_order)}

    def _normalize_item(x):
        # Expected: {value: 'USStock', i18nKey: '...'}
        if isinstance(x, dict):
            raw_value = (x.get('value') or '').strip()
            if not raw_value:
                return None
            try:
                v = normalize_supported_market(raw_value)
            except ValueError:
                return None
            return {
                'value': v,
...
def search_symbols():
    ...
    try:
        ...
        return jsonify({'code': 0, 'msg': str(e), 'data': []}), 400
    except Exception as e:
