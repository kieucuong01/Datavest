...
def _supported_position_identity(market, symbol):
    """Return the canonical supported market identity or ``None`` for retired data."""
    try:
        canonical_market = normalize_supported_market(market)
        canonical_symbol = canonicalize_supported_symbol(canonical_market, symbol)
...
def get_positions():
    ...
    try:
        ...
        for row in rows:
            identity = _supported_position_identity(row.get('market'), row.get('symbol'))
            if identity is None:
                continue
...
def add_position():
    ...
    try:
        ...
        identity = _supported_position_identity(market, symbol)
        if identity is None:
            return jsonify({'code': 0, 'msg': 'Only USStock, VNStock, Crypto, and Forex/XAU are supported', 'data': None}), 400
        market, symbol = identity
    ...
    try:
        ...
        for row in rows:
            ...
            if identity is None:
                continue
            item = dict(row)
            item['market'], item['symbol'] = identity
...
def run_monitor_now(monitor_id):
    ...
    try:
        ...
        language = detect_request_language(request, body=data)
        ...
        if async_mode:
            ...
            return jsonify({
                ...
                'data': {
                    'status': 'running',
                    'message': (
                        'Theo dõi đang chạy ở chế độ nền. Kết quả sẽ được gửi qua thông báo.'
                        if language == 'vi-VN'
                        else 'Monitor is running in background. Results will be sent via notification.'
                    )
                }
            })
        else:
