...
def publish_script_source():
    try:
        payload = _source_payload()
        try:
            requested_amount = float(payload.get("price") or 0)
        except Exception:
            requested_amount = 0
        if (
            str(payload.get("pricingType") or "free").strip().lower() != "free"
            or requested_amount > 0
            or bool(payload.get("vipFree") or payload.get("codeHidden"))
        ):
            return jsonify({
                "code": 0,
        ...
        ok, msg, data = get_community_library().publish_script_source(
            user_id=g.user_id,
            source_id=source_id,
            ...
            is_admin=is_admin,
            publication_id=int(payload.get("indicatorId") or 0),
        )
        if data is not None:
