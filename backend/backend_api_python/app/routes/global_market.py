...
Provides aggregated market data for the supported product universe:
- US stocks and Vietnamese stocks
- Crypto prices
- Gold (XAU) quotes
- Market heatmap data (crypto, stocks, gold)
...
@login_required
def market_overview():
    """Get the supported US/Vietnam/crypto/gold market overview."""
    try:
...
@login_required
def market_heatmap():
    """Get heatmap data for US/Vietnam stocks, crypto, gold, and US sectors."""
    try:
        force = request.args.get("force", "").lower() in ("true", "1")
        data = cached_or_compute(
...
def trading_opportunities():
    """Scan for trading opportunities across US/Vietnam stocks, crypto, and gold."""
    try:
        force = request.args.get("force", "").lower() in ("true", "1")
        data = cached_or_compute(
