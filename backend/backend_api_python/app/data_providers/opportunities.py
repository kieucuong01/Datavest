...
from app.utils.supported_markets import normalize_supported_market
...
def _fetch_single_local_stock_quote(market: str, item: Dict[str, Any], *, fast: bool = False) -> Optional[Dict[str, Any]]:
    """Fetch one US or Vietnamese stock quote row."""
...
def fetch_local_stock_opportunity_prices(market: str, limit: int = 15, *, fast: bool = False) -> List[Dict[str, Any]]:
    """Fetch US/Vietnam stock prices for opportunity scanning and heatmaps."""
    try:
        m = normalize_supported_market(market)
    except ValueError:
        return []
    if m not in ("VNStock", "USStock"):
        return []
    ...
    _FALLBACK_SYMBOLS = {
        ...
        "VNStock": [
            {"symbol": "FPT", "name": "FPT"}, {"symbol": "VCB", "name": "Vietcombank"},
            {"symbol": "HPG", "name": "Hoa Phat Group"}, {"symbol": "VIC", "name": "Vingroup"},
            {"symbol": "VHM", "name": "Vinhomes"}, {"symbol": "VNM", "name": "Vinamilk"},
            {"symbol": "SSI", "name": "SSI Securities"}, {"symbol": "MWG", "name": "Mobile World"},
            {"symbol": "VRE", "name": "Vincom Retail"}, {"symbol": "MSN", "name": "Masan Group"},
            {"symbol": "GAS", "name": "PV Gas"}, {"symbol": "BID", "name": "BIDV"},
            {"symbol": "CTG", "name": "VietinBank"}, {"symbol": "TCB", "name": "Techcombank"},
            {"symbol": "ACB", "name": "ACB"}, {"symbol": "MBB", "name": "MB Bank"},
    ...
    try:
        ...
        if m in ("USStock", "VNStock") and len(items) > 1:
...
def analyze_opportunities_local_stocks(opportunities: list, market: str):
    """Scan Vietnamese stocks for trading opportunities."""
    try:
        m = normalize_supported_market(market)
    except ValueError:
        return
    ...
    cache_key = "vn_stock_opportunity_prices"
