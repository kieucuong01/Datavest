...
def _build_ai_calendar_insight(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    title_vi = _event_outlook_title_vi(impact, released)
    pre_note_vi = _event_pre_release_note_vi(event_type, importance, window["label"])
    post_note_vi = _event_post_release_note_vi(event, impact)
...
def _event_outlook_title_vi(impact: str, released: bool) -> str:
    if impact == "bullish":
        return "Thiên hướng tích cực, nhưng cần lưu ý biến động mạnh trong thời điểm công bố"
    if impact == "bearish":
        return "Thiên hướng tiêu cực, rủi ro biến động trong thời điểm công bố cao"
    if released:
        return "Số liệu thực tế gần với dự báo; cần theo dõi phản ứng tiếp theo của thị trường"
    return "Ưu tiên rủi ro biến động trước khi công bố; chờ số liệu xác nhận xu hướng"
...
def _event_pre_release_note_vi(event_type: str, importance: str, window_label: str) -> str:
    type_vi = {
        "inflation": "lạm phát",
        "central_bank": "chính sách ngân hàng trung ương",
        "jobs": "việc làm",
        "growth": "tăng trưởng",
        "energy": "năng lượng",
        "earnings": "kết quả kinh doanh",
        "macro": "vĩ mô",
    }.get(event_type, "vĩ mô")
    risk_vi = "tác động cao" if importance == "high" else "tác động trung bình" if importance == "medium" else "tác động thấp"
    return (
        f"Sự kiện {type_vi} có {risk_vi}. Theo dõi cửa sổ biến động {window_label}; "
        "không nên chạy theo giá trước khi công bố và chỉ đánh giá hướng đi sau khi so sánh số liệu thực tế với dự báo."
    )
...
def _event_post_release_note_vi(event: Dict[str, Any], impact: str) -> str:
    actual = event.get("actual") or "-"
    forecast = event.get("forecast") or "-"
    previous = event.get("previous") or "-"
    if impact == "bullish":
        direction = "Số liệu tương đối tích cực"
    elif impact == "bearish":
        direction = "Số liệu tương đối tiêu cực"
    else:
        direction = "Số liệu gần với dự báo"
    return (
        f"{direction}: thực tế {actual}, dự báo {forecast}, kỳ trước {previous}. "
        "Nên kết hợp khối lượng và khả năng duy trì xu hướng sau công bố để đánh giá."
    )
...
def _asset_payload(symbol: str, event_type: str, impact: str) -> Dict[str, str]:
    ...
    return {
        ...
        "label_vi": meta.get("label_vi", meta["label_en"]),
        "bias": bias,
        "bias_label": _bias_label(bias),
        "bias_label_en": _bias_label_en(bias),
        "bias_label_vi": _bias_label_vi(bias),
    }
...
def _bias_label_vi(bias: str) -> str:
    return {
        "up": "thiên hướng tăng",
        "down": "thiên hướng giảm",
