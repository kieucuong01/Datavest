"""Heatmap data aggregator for the supported US/Vietnam/crypto/gold scope."""
...
def _fetch_vn_stocks() -> List[Dict[str, Any]]:
    data = get_cached("vn_stock_heatmap_prices")
    if not data or len(data) < HEATMAP_CELL_COUNT:
        data = fetch_local_stock_opportunity_prices("VNStock", limit=HEATMAP_CELL_COUNT, fast=True)
        if data:
            set_cached("vn_stock_heatmap_prices", data, 300)
            set_cached("vn_stock_opportunity_prices", data, 300)
    return _stock_rows(data)


def _fetch_gold_heatmap() -> List[Dict[str, Any]]:
...
def _fetch_gold_from_forex_heatmap() -> List[Dict[str, Any]]:
    """Render XAU from the Forex adapter as the product's only FX instrument."""
    ...
    for pair in forex_data or []:
        rows.append({
            "name": pair.get("symbol", pair.get("name", "")),
...
def _fetch_indices_heatmap() -> List[Dict[str, Any]]:
    # Global indices are outside the product universe. Keep this helper as an
    # explicit empty section so stale cached index rows cannot leak into UI.
    return []


def generate_heatmap_data() -> Dict[str, Any]:
    """Generate heatmap data for US/Vietnam stocks, crypto, gold, and US sectors."""
    with ThreadPoolExecutor(max_workers=6) as pool:
        fut_us = pool.submit(_fetch_us_stocks)
        fut_vn = pool.submit(_fetch_vn_stocks)
        fut_crypto = pool.submit(_build_crypto_heatmap)
        fut_gold = pool.submit(_fetch_gold_heatmap)
        fut_gold_fx = pool.submit(_fetch_gold_from_forex_heatmap)
        fut_sectors = pool.submit(_fetch_sectors_heatmap)

        return {
            "us_stocks": fut_us.result(),
            "vn_stocks": fut_vn.result(),
            "crypto": fut_crypto.result(),
            "gold": fut_gold.result() or fut_gold_fx.result(),
