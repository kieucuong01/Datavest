...
An indicator cannot place orders, backtest, read an account, manage positions, enable leverage, or execute stop-loss/take-profit rules. To test an idea, convert its visual signals into a Strategy API V2 strategy, then verify and backtest that strategy separately.
...
| --- | --- | --- |
| Chart Indicator | plots, panes, lamp rows, visual markers, zones, labels | backtest, orders, positions, leverage, execution risk |
| Strategy API V2 | subscriptions, signals, simulated order intents, positions, backtest, protections | Indicator IDE <code>output</code> rendering |
