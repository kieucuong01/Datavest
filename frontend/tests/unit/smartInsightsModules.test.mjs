...
test('Smart Insights view and VI/EN messages expose every required module and unavailable state', async () => {
  ...
  assert.match(component, /calendarError/)
  assert.match(component, /retrySection/)
  assert.doesNotMatch(component, /data-readiness|dataReadiness|healthVisible|loadHealth/u)
...
test('Smart Insights opens on the AI Assistant daily brief with the crypto pulse loaded', async () => {
  const component = readFileSync(pagePath, 'utf8')
  assert.match(component, /getSmartInsightsOverview/u)
  assert.match(component, /dailyBrief/u)
