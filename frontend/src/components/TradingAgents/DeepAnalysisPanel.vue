...
export default {
  ...
  computed: {
    ...
    progressPercent () {
      const count = Array.isArray(this.run && this.run.events) ? this.run.events.length : 0
      return Math.min(92, Math.max(12, 12 + count * 7))
    },
    latestEventLabel () {
      const events = Array.isArray(this.run && this.run.events) ? this.run.events : []
      const latest = events[events.length - 1]
      if (!latest) return this.$t('tradingAgents.waitingForGraph')
      return latest.event_type === 'tool' ? this.$t('tradingAgents.toolStep') : this.$t('tradingAgents.graphStep')
    },
  ...
  methods: {
    ...
    async start () {
      if (!this.isSupported || this.starting || this.historyLoading) return
      this.starting = true
      this.errorMessage = ''
      this.reportContent = ''
      try {
        ...
        this.run = { run_id: data.run_id, status: data.status || 'queued', events: [] }
        this.startPolling()
      } catch (error) {
    ...
    async refreshRun () {
      ...
      try {
        const data = this.unwrap(await getTradingAgentsRun(this.run.run_id))
    ...
    async loadLatestRun () {
      if (!this.visible || !this.isSupported || this.historyLoading) return
      ...
      try {
        ...
        if (!summaries.length || !summaries[0].run_id) return
        const detail = this.unwrap(await getTradingAgentsRun(summaries[0].run_id))
        ...
        if (requestId === this.historyRequestId && this.visible) {
          this.errorMessage = (error && error.backendMessage) || (error && error.message) || this.$t('tradingAgents.historyLoadFailed')
        }
      } finally {
    ...
    async loadReport (run) {
      ...
      if (!artifact) return
      try {
        const response = await getTradingAgentsArtifact(run.run_id, artifact.artifact_name)
...
@media (max-width: 640px) { .deep-analysis-context { flex-direction: column; gap: 10px; }.deep-analysis-context h3 { font-size: 18px; }.deep-analysis-provenance { justify-content: flex-start; }.deep-analysis-empty { min-height: 230px; padding: 28px 12px; }.deep-analysis-status-row { align-items: flex-start; flex-direction: column; }.deep-analysis-progress { grid-template-columns: 1fr; }.deep-analysis-progress .ant-btn { width: 100%; min-height: 44px; }.recovery-actions { display: grid; grid-template-columns: 1fr; }.recovery-actions .ant-btn { min-height: 44px; }.report-heading { align-items: flex-start; flex-direction: column; }.deep-analysis-report pre { max-height: 48vh; padding: 13px; font-size: 11px; }.deep-analysis-footer { align-items: stretch; flex-direction: column; }.deep-analysis-footer .ant-btn { min-height: 44px; } }
</style>
