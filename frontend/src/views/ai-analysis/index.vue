          ...
          <div v-if="false" class="heatmap-box">
            <div class="box-header">
              <a-radio-group v-model="heatmapType" size="small" button-style="solid" class="heatmap-type-tabs">
                ...
                <a-radio-button value="vn_stocks">{{ $t('globalMarket.vnStockHeatmap') }}</a-radio-button>
                <a-radio-button value="crypto">{{ $t('globalMarket.cryptoHeatmap') }}</a-radio-button>
                <a-radio-button value="gold">{{ $t('globalMarket.goldHeatmap') }}</a-radio-button>
              ...
              <template v-else-if="marketData.calendar.length > 0">
                <div v-for="evt in marketData.calendar.slice(0, 6)" :key="`${evt.date}-${evt.time}-${evt.name_en}-${evt.country}`" class="cal-item" :class="evt.importance">
                  <div class="cal-main-row">
                    ...
                    <span class="cal-name">{{ getCalendarEventName(evt) }}</span>
                  ...
                  <div v-if="evt.ai_insight" class="cal-ai-insight" :class="evt.ai_insight.risk_level">
                    ...
                    <div class="cal-ai-assets" v-if="getCalendarInsightAssets(evt).length">
                      <span
                        ...
                        :class="asset.bias"
                      >
                        {{ asset.symbol }} · {{ formatCalendarBias(asset.bias) }}
                      </span>
                    </div>
                  </div>
                </div>
...
class="analyze-button">
          ...
          <div class="analysis-main">
            <div v-if="!analysisResult && !analyzing && !analysisError" class="analysis-placeholder">
              <div class="placeholder-hero">
                ...
                <div class="hero-body">
                  <div class="hero-badge">{{ $t('aiAssetAnalysis.aiPowered') }}</div>
    ...
    <a-modal
      ...
      :title="`${$t('dashboard.analysis.position.createTitle')} - ${targetStockForOps ? targetStockForOps.symbol : ''}`"
      @ok="savePosition"
    ...
    >
      <a-form layout="vertical">
        ...
        <a-form-item :label="$t('dashboard.analysis.position.quantity')">
          <a-input-number v-model="positionForm.quantity" :min="0" :step="0.01" style="width: 100%;" />
        </a-form-item>
        <a-form-item :label="$t('dashboard.analysis.position.entryPrice')">
...
export default {
  ...
  data () {
    return {
      ...
      marketData: {
        ...
        dxy: null,
        indices: [],
        heatmap: { us_stocks: [], vn_stocks: [], crypto: [], gold: [], sectors: [] },
        calendar: []
      },
      calendarMeta: {},
  ...
  computed: {
    ...
    isVietnameseLocale () {
      return String(this.$i18n.locale || '').toLowerCase().startsWith('vi')
    },
    calendarStatus () {
    ...
    searchInputPlaceholder () {
      ...
      const examples = {
        ...
        VNStock: this.i18nText('dashboard.analysis.modal.addStock.example.VNStock', 'e.g. FPT, VCB, HPG'),
        Crypto: this.i18nText('dashboard.analysis.modal.addStock.example.Crypto', 'e.g. BTC/USDT, ETH/USDT, SOL/USDT'),
        Forex: this.i18nText('dashboard.analysis.modal.addStock.example.Forex', 'e.g. XAUUSD, XAU/USD')
      }
  ...
  methods: {
    ...
    async savePosition () {
      ...
      if (!(quantity > 0) || !(entryPrice > 0)) {
        this.$message.warning(this.$t('dashboard.analysis.position.invalidValues'))
        return
      }
      try {
    ...
    formatHeatmapPrice (price) {
      if (!price) return ''
      const prefix = this.heatmapType === 'vn_stocks' ? '₫' : '$'
    ...
    getHeatmapName (item) {
      if (this.heatmapType === 'vn_stocks') {
        return this.isVietnameseLocale ? (item.name_vi || item.name || item.fullName) : (item.name_en || item.name || item.fullName)
      }
      if (this.heatmapType === 'us_stocks') {
        return item.name || item.fullName || ''
      }
      if (this.heatmapType === 'sectors' || this.heatmapType === 'gold') {
        return this.isVietnameseLocale ? (item.name_vi || item.name_en || item.name) : (item.name_en || item.name)
      }
      return item.name
    },
    getCalendarEventName (evt) {
      if (!evt) return ''
      if (this.isVietnameseLocale) return evt.name_vi || evt.name_en || evt.name || ''
      return evt.name_en || evt.name || ''
    },
    formatCalendarBias (bias) {
      const value = String(bias || '').toLowerCase()
      if (value === 'bullish' || value === 'positive' || value === 'buy') return this.$t('fastAnalysis.outlookBull')
      if (value === 'bearish' || value === 'negative' || value === 'sell') return this.$t('fastAnalysis.outlookBear')
    ...
    getCalendarInsightTitle (evt) {
      ...
      if (!insight) return ''
      return this.isVietnameseLocale ? (insight.title_vi || insight.title_en || insight.title || '') : (insight.title_en || insight.title || '')
    },
    getCalendarInsightSummary (evt) {
      const insight = evt && evt.ai_insight
      if (!insight) return ''
      return this.isVietnameseLocale ? (insight.summary_vi || insight.summary_en || insight.summary || '') : (insight.summary_en || insight.summary || '')
    },
    ...
    neutralizeDecisionText (text) {
      ...
      const prefix = this.$t('fastAnalysis.outlookPrefix')
      return String(text)
        .replace(/建议\s*BUY/gi, `${prefix} ${buy}`)
        .replace(/建议\s*SELL/gi, `${prefix} ${sell}`)
        .replace(/建议\s*HOLD/gi, `${prefix} ${hold}`)
        .replace(/suggested\s+decision\s+BUY/gi, `${prefix} ${buy}`)
        .replace(/suggested\s+decision\s+SELL/gi, `${prefix} ${sell}`)
        .replace(/suggested\s+decision\s+HOLD/gi, `${prefix} ${hold}`)
    ...
    async startFastAnalysis () {
      ...
      const language = this.$store.getters.lang || this.$i18n.locale || 'en-US'
    ...
    _validateWatchlistPair (market, symbol) {
      ...
      try {
        const normalizedMarket = normalizeSupportedMarket(market)
        const normalizedSymbol = canonicalizeSupportedSymbol(normalizedMarket, symbol)
        if (normalizedMarket === 'Crypto' && !normalizedSymbol.includes('/')) {
          return this.i18nText('dashboard.analysis.modal.addStock.mismatch.cryptoPairHint', 'Crypto symbols should be entered as BASE/QUOTE, such as BTC/USDT.')
        }
      } catch (error) {
        return error && error.message ? error.message : this.i18nText('dashboard.analysis.modal.addStock.invalidSymbol', 'Unsupported market or symbol')
      }
      return ''
    },
    ...
    isUsableSymbolSearchResult (item) {
      ...
      if (market === 'VNStock') return /^[A-Z][A-Z0-9.-]{0,9}$/.test(symbol)
      if (market === 'USStock') return /^[A-Z][A-Z0-9.-]{0,9}$/.test(symbol)
      if (market === 'Crypto') return /^[A-Z0-9]+\/[A-Z0-9]+$/.test(symbol)
      if (market === 'Forex') return symbol === 'XAU' || symbol === 'XAUUSD' || symbol === 'XAU/USD'
      return false
    },
