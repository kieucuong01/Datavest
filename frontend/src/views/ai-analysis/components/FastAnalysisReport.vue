      ...
      <div v-if="isCryptoResult && cryptoFactorRows.length" class="report-section">
        <div class="report-section-header" @click="toggleSection('cryptoFactors')">
          <span class="rsh-title">
            <a-icon type="fund" />
            {{ $t('fastAnalysis.cryptoStructure') }}
          </span>
...
export default {
  ...
  computed: {
    ...
    trendOutlookBlocks () {
      ...
      return keys.map(({ key, labelKey }) => {
        ...
        return {
          ...
          trend: block.trend,
          score: block.score,
          strength: this.translateOutlookStrength(block.strength)
        }
      }).filter(r => {
        const b = o[r.key]
    ...
    cryptoFactorRows () {
      ...
      add('volume_24h', this.$t('fastAnalysis.factor.volume24h'), usd(cf.volume_24h))
      add('volume_change_24h', this.$t('fastAnalysis.factor.volumeChange'), pct(cf.volume_change_24h), Number(cf.volume_change_24h) > 0 ? 'bullish' : (Number(cf.volume_change_24h) < 0 ? 'bearish' : ''))
      add('funding_rate', this.$t('fastAnalysis.factor.fundingRate'), pct(cf.funding_rate), Number(cf.funding_rate) > 0 ? 'bullish' : (Number(cf.funding_rate) < 0 ? 'bearish' : ''))
      add('open_interest', this.$t('fastAnalysis.factor.openInterest'), usd(cf.open_interest))
      add('open_interest_change_24h', this.$t('fastAnalysis.factor.openInterestChange'), pct(cf.open_interest_change_24h), Number(cf.open_interest_change_24h) > 0 ? 'bullish' : (Number(cf.open_interest_change_24h) < 0 ? 'bearish' : ''))
      add('long_short_ratio', this.$t('fastAnalysis.factor.longShortRatio'), this.formatCompactNum(cf.long_short_ratio))
      add('exchange_netflow', this.$t('fastAnalysis.factor.exchangeNetflow'), usd(cf.exchange_netflow), Number(cf.exchange_netflow) < 0 ? 'bullish' : (Number(cf.exchange_netflow) > 0 ? 'bearish' : ''))
      add('stablecoin_netflow', this.$t('fastAnalysis.factor.stablecoinNetflow'), usd(cf.stablecoin_netflow), Number(cf.stablecoin_netflow) > 0 ? 'bullish' : (Number(cf.stablecoin_netflow) < 0 ? 'bearish' : ''))
      return rows
    },
    cryptoSignals () {
      if (!this.isCryptoResult) return []
      ...
      if (sig.derivatives_bias) {
        items.push({
          ...
          label: `${this.$t('fastAnalysis.factor.derivatives')}: ${this.translateCryptoBias(sig.derivatives_bias)}`
        })
      }
      if (sig.flow_bias) {
        items.push({
          ...
          label: `${this.$t('fastAnalysis.factor.flow')}: ${this.translateCryptoBias(sig.flow_bias)}`
        })
      }
      if (sig.squeeze_risk) {
        items.push({
          ...
          label: `${this.$t('fastAnalysis.factor.squeezeRisk')}: ${this.translateCryptoBias(sig.squeeze_risk)}`
        })
      }
      return items
    },
    ...
    cryptoFactorScoreText () {
      const score = Number(this.result?.crypto_factor_score)
      if (Number.isNaN(score)) return this.$t('fastAnalysis.bias.insufficient')
      if (score >= 40) return this.$t('fastAnalysis.bias.bullish')
      if (score >= 20) return this.$t('fastAnalysis.bias.mildBullish')
      if (score <= -40) return this.$t('fastAnalysis.bias.bearish')
      if (score <= -20) return this.$t('fastAnalysis.bias.mildBearish')
      return this.$t('fastAnalysis.bias.neutral')
    },
    errorTitle () {
  ...
  methods: {
    ...
    translateCryptoBias (value) {
      ...
      return key ? this.$t(`fastAnalysis.bias.${key}`) : value
    },
    ...
    translateOutlookStrength (strength) {
      const normalized = String(strength || '').toLowerCase()
      const key = {
        strong: 'fastAnalysis.outlookStrengthStrong',
        moderate: 'fastAnalysis.outlookStrengthModerate',
        mild: 'fastAnalysis.outlookStrengthMild',
        neutral: 'fastAnalysis.outlookStrengthNeutral'
      }[normalized]
      return key ? this.$t(key) : (strength || '--')
    },
