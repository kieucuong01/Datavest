...
export default {
  ...
  computed: {
    ...
    isVietnamese () {
      const locale = this.$i18n ? String(this.$i18n.locale || '') : 'en-US'
    ...
    text () {
      ...
      return {
        ...
        streamInterrupted: t('streamInterrupted', 'The connection was interrupted. The current response was kept; please retry.'),
        streamIncomplete: t('streamIncomplete', 'The response did not finish correctly. Please retry.'),
        outputLimit: t('outputLimit', 'The response reached the output limit and may be incomplete.'),
  ...
  methods: {
    ...
    buildPreflightGuide (task = null) {
      ...
      const blockers = (Array.isArray(status.blockers) ? status.blockers : [])
        .filter(item => !this.isRetiredBillingPreflightItem(item))
    ...
    isRetiredBillingPreflightItem (item) {
    ...
    inferSymbolFromText (text) {
      ...
      const marketPair = value.match(/\b(CRYPTO|USSTOCK|VNSTOCK|FOREX):([A-Z0-9./_-]{2,24})\b/)
    ...
    strategyPromptTarget (target = {}, entities = {}) {
      ...
      if (market.toLowerCase() !== 'crypto') {
        return /^(?:VNStock|Forex|USStock):/i.test(rawSymbol)
          ? rawSymbol
          : `${market}:${rawSymbol}`
      }
      ...
      if (!target || !target.symbol) {
        this.messages.push({
          ...
          content: this.i18nText(
            ...
            'I classified this as a strategy creation task, but the target symbol is missing. Please provide a symbol such as `Crypto:BTC/USDT`, `USStock:SPCX`, `VNStock:FPT`, or `Forex:XAUUSD`.'
          ),
    ...
    eventTitle (event) {
      ...
      if (this.isVietnamese) return event.title_vi || event.name_vi || event.title_en || event.name_en || event.title || event.event || event.name || '--'
    ...
    renderMarkdown (text) {
      ...
      const withTokens = source.replace(/```([\w+-]*)\n?([\s\S]*?)```/g, (_, lang, code) => {
        ...
        blocks.push(
          ...
          `<div class="qd-code-head"><span>${this.escapeHtml(label)}</span><button type="button" class="qd-copy-code" data-code="${encodeURIComponent(code)}">${this.escapeHtml(this.$t('common.copy'))}</button></div>` +
    ...
    buildSetupGuide (error, context = {}) {
      ...
      if (type === 'broker') {
        return guide(
          'broker',
          'Setup check: unavailable integration',
          'This integration is unavailable',
          'This research-only workspace does not expose broker account connections.',
          []
        )
      }
      if (type === 'auth') {
      ...
      return guide(
        ...
        'Check AI/LLM and data source settings, then retry.',
        ['ai', 'data']
      )
    },
    scrollToBottom () {
