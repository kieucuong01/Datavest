...
<script>
import { getProfile, updateProfile, getLoginLogs, getMyReferrals, getNotificationSettings, updateNotificationSettings, testNotificationSettings, getMfaStatus, startMfaSetup, confirmMfaSetup, disableMfa } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    applyTabFromQuery (rawTab) {
      const allowed = ['basic', 'password', 'security', 'notifications', 'referrals', 'loginLogs']
    ...
    async loadProfile () {
      ...
      try {
        ...
        this.$message.error(this.$t('profile.loadFailed'))
      } finally {
        this.loading = false
      }
    },
    ...
    handleSaveProfile () {
      this.profileForm.validateFields(async (err, values) => {
        ...
        try {
          ...
          this.$message.error(this.$t('profile.updateFailed'))
        } finally {
          this.saving = false
        }
      })
    ...
    async loadReferrals () {
      ...
      try {
        ...
        this.$message.error(this.$t('profile.referral.loadFailed'))
      } finally {
