...
export default {
  ...
  methods: {
    ...
    async loadSkills () {
      const res = await getAiSkills({ language: this.productLanguage, include_disabled: this.showDisabled ? 1 : 0 })
      this.registry = res.data || res
    },
    async loadTools () {
      const res = await getAiTools({ language: this.productLanguage })
    ...
    async toggleSkill (record, checked) {
      try {
        await updateAiSkill(record.id, { enabled: checked, language: this.productLanguage })
    ...
    async installSkill () {
      ...
      try {
        await installAiSkill({ skill: payload, language: this.productLanguage })
