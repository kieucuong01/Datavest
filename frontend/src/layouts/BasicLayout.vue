<template>
  <div :class="['basic-layout-wrapper', settings.theme, { 'basic-layout-wrapper--multi-tab': multiTab }]">
    ...
    >
      ...
      <live-data-sources
        :rows="liveAssetRows"
        :fetched-at="liveAssetsFetchedAt"
        :loading="liveAssetsLoading"
        :error="liveAssetError"
      />
...
import logoLight from '@/assets/datavest-logo-light.svg'
import logoDark from '@/assets/datavest-logo-dark.svg'
import slogoImg from '@/assets/datavest-mark.svg'
import { getSmartInsightsLiveAssets } from '@/api/smart-insights'
import { normalizeLiveAssetRows } from '@/views/smart-insights/liveAssets'
  ...
  computed: {
    ...
    activeTopMenuKey () {
      ...
      const isActive = route => {
        ...
        return Boolean(routePath) && (routePath === currentPath || currentPath.startsWith(`${routePath}/`))
      }
      ...
      return match ? match.path : currentPath
    }
  },
  created () {
  ...
  methods: {
    async loadLiveAssets () {
      ...
      try {
        ...
        this.liveAssets = Array.isArray(payload.assets) ? payload.assets : []
        this.liveAssetsFetchedAt = payload.fetchedAt || ''
        this.liveAssetError = ''
      } catch (error) {
        this.liveAssetError = this.$i18n && this.$i18n.locale === 'vi-VN'
          ? this.$t('smartInsights.watchlistUnavailable')
    ...
    topMenuRender (h, props) {
      ...
      return h(TopMenu, {
        props: {
          menus: this.menus,
          selectedKey: this.activeTopMenuKey,
    ...
    buildTopMenuGroups (routes) {
      ...
      const topAdminMenuPaths = ['/ai-skills', '/settings']
      ...
      const groups = [
        {
          name: 'MenuGroupSmartInsights',
          path: '/smart-insights',
          title: 'menu.group.smartInsights',
          icon: 'bulb',
          paths: ['/smart-insights'],
          singleAsItem: true
        },
        {
          name: 'MenuGroupAI',
        ...
        {
          name: 'MenuGroupQuantLab',
          path: '/menu-group/quant-lab',
          title: 'menu.group.quantLab',
          icon: 'experiment',
          paths: ['/portfolio-optimizer', '/strategy-ide', '/backtest-center'],
          singleAsItem: false
        }
      ]
