...
export function getPublicResearchReportHistory (assetKey, reportKind = 'deep') {
  return request({
    url: `/api/smart-insights/public/reports/${encodeURIComponent(String(assetKey || ''))}/${encodeURIComponent(String(reportKind || ''))}/history`,
    method: 'get',
    params: { lang: 'vi-VN' }
  })
}

export function getPublicResearchReportPdf (assetKey, revision, effectiveDate) {
  return request({
    url: `/api/smart-insights/public/reports/${encodeURIComponent(String(assetKey || ''))}/deep.pdf`,
    method: 'get',
    params: { lang: 'vi-VN', r: revision, date: effectiveDate },
    responseType: 'blob',
    timeout: 120000
  })
}

export function getPublicResearchReportSummaryPdf (assetKey, revision, effectiveDate) {
  return request({
